...
def _get_config() -> Dict[str, Any]:
    ...
    if not os.path.exists(config_path):
        raise KnownException(
            f"Config file {config_path} does not exist, please copy and modify example"
        )
    with open(config_path, "r", encoding="utf8") as config_file:
        try:
            ...
            raise KnownException(
                f"Config file {config_path} is not valid YAML, please copy and modify example"
            )
    if not config:
        raise KnownException(
            f"Config file {config_path} is invalid, please copy and modify example"
        )
    ...
    if (
        not dc_config
        or not dc_config.get("network")
        or not dc_config.get("project")
        or not dc_config.get("registry")
    ):
        raise KnownException(
    ...
    for project_name in projects:
        ...
        if (
            ...
            or "directory" not in project_data
            or "repository" not in project_data
            or "services" not in project_data
            or not isinstance(project_data.get("services"), list)
        ):
            raise KnownException(
...
def get_target_service(target_name: str, optional: bool = False) -> Optional[str]:
    ...
    config = _get_config()
    if (
        not config.get("target-services")
        or target_name not in config["target-services"]
    ):
        if optional:
            return None
        raise KnownException(
            f'Target service {target_name} is not defined, please define "target-services"'
        )
